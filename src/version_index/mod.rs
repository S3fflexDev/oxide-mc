...
pub async fn find_version_manifest_url(version_id: &str) -> anyhow::Result<String> {
    ...
    let response_text = client.get(VERSION_MANIFEST_URL)
        .send()
        .await?
        .text()
        .await?;
    ...
    match found_version {
        Some(version_info) => {
            println!("Version '{}' found. Manifest URL: {}", version_id, version_info.url);
            Ok(version_info.url.clone())
        }
        None => {
            Err(anyhow::anyhow!("Version '{}' not found in the official manifest.", version_id))
        }
    }
}